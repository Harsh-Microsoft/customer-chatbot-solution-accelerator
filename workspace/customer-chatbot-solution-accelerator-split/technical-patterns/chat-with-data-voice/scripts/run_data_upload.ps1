...
if (-not $resource_group) {
    if (-not (Get-ValuesFromAzdEnv)) {
        ...
        Write-Host "Usage: .\run_data_upload.ps1 -resource_group <ResourceGroupName>"
        exit 1
    }
} else {
...
try {
    ...
    for ($attempt = 1; $attempt -le $maxUploadAttempts; $attempt++) {
        $uploadOutput = & python -m scripts.data.upload_catalog_data --cosmosdb_account="$cosmosdb_account" 2>&1
    ...
    if ($cosmosAccessEnabled) {
        Write-Host "Restoring Azure Cosmos DB network access to: $originalCosmosPublicAccess"
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Azure Cosmos DB network access restored."
        } else {
            Write-Host "Warning: Could not restore Azure Cosmos DB network access. Please restore it manually in Azure Portal."
        }
    }
...
Write-Host "Data upload script completed successfully."
