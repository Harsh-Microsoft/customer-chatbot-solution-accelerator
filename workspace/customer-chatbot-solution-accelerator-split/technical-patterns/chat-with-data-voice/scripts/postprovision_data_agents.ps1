...
try {
  & (Join-Path $repoRoot 'scripts/run_data_upload.ps1')
...
try {
  & (Join-Path $repoRoot 'scripts/run_agents.ps1')
  if ($LASTEXITCODE -ne 0) {
...
if (-not $isUploadDataScriptSuccess -or -not $isCreateAgentsScriptSuccess) {
  ...
  if (-not $isUploadDataScriptSuccess) {
    ...
    Write-Host "`n    scripts/run_data_upload.ps1`n" -ForegroundColor Yellow
  }
  ...
  if (-not $isCreateAgentsScriptSuccess) {
    ...
    Write-Host "`n    scripts/run_agents.ps1`n" -ForegroundColor Yellow
  }
