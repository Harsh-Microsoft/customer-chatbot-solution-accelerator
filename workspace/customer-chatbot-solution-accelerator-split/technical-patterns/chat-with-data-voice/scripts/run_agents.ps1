...
if (-not $resourceGroup) {
    if (-not (Get-ValuesFromAzdEnv)) {
        ...
        Write-Host "Usage: .\run_agents.ps1 -resourceGroup <ResourceGroupName>"
        exit 1
    }
...
try {
    ...
    $pythonLines = @(python -m scripts.agents.run_agents --ai_project_endpoint="$projectEndpoint" --solution_name="$solutionName" --gpt_model_name="$gptModelName" --ai_search_endpoint="$searchEndpoint" 2>&1)
    ...
    foreach ($line in $pythonLines) {
        ...
        elseif ($lineStr -match "^catalogAgentName=(.+)$") {
            $catalogAgentName = $Matches[1]
        }
    ...
    Write-Host "Catalog Agent Name: $catalogAgentName"
