...
# App Service discovery prefixes, are now parameters instead of hardcoded
# so this script also works for technical-patterns/chat-with-data-voice's
...
param (
    ...
    # Root of the solution folder containing src/; defaults to the caller's working directory.
    [string]$SolutionPath = $PWD,
    # Image name / build-context (relative to SolutionPath) / Dockerfile triples, per app.
    [string]$WebImageName = "da-app",
    [string]$WebContext = "src/App",
    [string]$WebDockerfile = "WebApp.Dockerfile",
    [string]$ApiImageName = "da-api",
    [string]$ApiContext = "src/api/python",
    [string]$ApiDockerfile = "ApiApp.Dockerfile",
    [string]$ApiDotnetImageName = "da-api-dotnet",
    [string]$ApiDotnetContext = "src/api/dotnet",
    [string]$ApiDotnetDockerfile = "CsApi.Dockerfile",
    # App Service name prefixes used for auto-discovery.
    [string]$ApiAppPrefix = "api-",
    [string]$WebAppPrefix = "app-",
    [string]$ApiDotnetAppPrefix = "api-cs-",
...
Options:
  ...
  -SolutionPath <path>         Root of the solution folder containing src/ (default: current working directory)
  -WebImageName <name>         Frontend image name (default: da-app)
  -WebContext <path>           Frontend build context, relative to SolutionPath (default: src/App)
  -WebDockerfile <file>        Frontend Dockerfile, relative to WebContext (default: WebApp.Dockerfile)
  -ApiImageName <name>         Backend (python) image name (default: da-api)
  -ApiContext <path>           Backend (python) build context, relative to SolutionPath (default: src/api/python)
  -ApiDockerfile <file>        Backend (python) Dockerfile, relative to ApiContext (default: ApiApp.Dockerfile)
  -ApiDotnetImageName <name>   Backend (dotnet) image name (default: da-api-dotnet)
  -ApiDotnetContext <path>     Backend (dotnet) build context, relative to SolutionPath (default: src/api/dotnet)
  -ApiDotnetDockerfile <file>  Backend (dotnet) Dockerfile, relative to ApiDotnetContext (default: CsApi.Dockerfile)
  -ApiAppPrefix <prefix>       App Service name prefix for backend discovery (default: api-)
  -WebAppPrefix <prefix>       App Service name prefix for frontend discovery (default: app-)
  -ApiDotnetAppPrefix <prefix> App Service name prefix for dotnet backend discovery (default: api-cs-)
...
if (-not $ResourceGroup) {
    Write-Error "-ResourceGroup is required."
    Show-Usage
    exit 1
}

# ----------------------------------------------------------------------------
# Resolve and validate SolutionPath (must contain src/)
# ----------------------------------------------------------------------------
$SolutionPath = (Resolve-Path $SolutionPath).Path
if (-not (Test-Path (Join-Path $SolutionPath "src"))) {
    Write-Error "SolutionPath '$SolutionPath' does not contain a src/ folder. Pass -SolutionPath pointing to the solution root."
    exit 1
}
...
if (-not $ApiAppName) {
    $dotnetApp = az webapp list --resource-group $ResourceGroup --query "[?starts_with(name, '$ApiDotnetAppPrefix')].name | [0]" -o tsv 2>$null
    if ($dotnetApp) {
        $ApiAppName = $dotnetApp
        $BackendRuntime = "dotnet"
    }
    else {
        $ApiAppName = az webapp list --resource-group $ResourceGroup --query "[?starts_with(name, '$ApiAppPrefix') && !starts_with(name, '$ApiDotnetAppPrefix')].name | [0]" -o tsv 2>$null
    ...
    if ($ApiAppName) {
        ...
        Write-Warn "No Backend App Service ($ApiAppPrefix* / $ApiDotnetAppPrefix*) found in '$ResourceGroup'."
    }
}

if (-not $WebAppName) {
    $WebAppName = az webapp list --resource-group $ResourceGroup --query "[?starts_with(name, '$WebAppPrefix')].name | [0]" -o tsv 2>$null
    if ($WebAppName) {
        Write-Success "Discovered Frontend App : $WebAppName"
    }
    else {
        Write-Warn "No Frontend App Service ($WebAppPrefix*) found in '$ResourceGroup'."
    }
}
...
$Images = @(
    [pscustomobject]@{ Name = $WebImageName; Context = "$SolutionPath/$WebContext"; Dockerfile = $WebDockerfile }
)
if ($BackendRuntime -eq "dotnet") {
    $Images += [pscustomobject]@{ Name = $ApiDotnetImageName; Context = "$SolutionPath/$ApiDotnetContext"; Dockerfile = $ApiDotnetDockerfile }
    $BackendImage = $ApiDotnetImageName
}
else {
    $Images += [pscustomobject]@{ Name = $ApiImageName; Context = "$SolutionPath/$ApiContext"; Dockerfile = $ApiDockerfile }
    $BackendImage = $ApiImageName
}
...
try {
    ...
    Update-AppServiceImage -AppName $WebAppName -ImageName $WebImageName
