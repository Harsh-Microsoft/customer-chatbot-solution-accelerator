...
#
# STAGED DELTA (see technical-patterns/.shared/README.md): the image name,
# build-context, and Dockerfile for each app, plus the api-/app-/api-cs-
# App Service discovery prefixes, are now flags instead of hardcoded so
# this script also works for technical-patterns/chat-with-data-voice's
# src/api + src/app layout without forking. Defaults are unchanged and
# match the original chat-with-data layout (src/App, src/api/python).
# ============================================================================
set -euo pipefail
_AZ_PY=""
...
WEB_IMAGE_NAME="da-app"
WEB_CONTEXT="src/App"
WEB_DOCKERFILE="WebApp.Dockerfile"
API_IMAGE_NAME="da-api"
API_CONTEXT="src/api/python"
API_DOCKERFILE="ApiApp.Dockerfile"
API_DOTNET_IMAGE_NAME="da-api-dotnet"
API_DOTNET_CONTEXT="src/api/dotnet"
API_DOTNET_DOCKERFILE="CsApi.Dockerfile"
API_APP_PREFIX="api-"
WEB_APP_PREFIX="app-"
API_DOTNET_APP_PREFIX="api-cs-"
...
Options:
  ...
  --solution-path <path>         Root of the solution folder containing src/ (default: current working directory)
  --web-image-name <name>        Frontend image name (default: da-app)
  --web-context <path>           Frontend build context, relative to solution-path (default: src/App)
  --web-dockerfile <file>        Frontend Dockerfile, relative to web-context (default: WebApp.Dockerfile)
  --api-image-name <name>        Backend (python) image name (default: da-api)
  --api-context <path>           Backend (python) build context, relative to solution-path (default: src/api/python)
  --api-dockerfile <file>        Backend (python) Dockerfile, relative to api-context (default: ApiApp.Dockerfile)
  --api-dotnet-image-name <name> Backend (dotnet) image name (default: da-api-dotnet)
  --api-dotnet-context <path>    Backend (dotnet) build context, relative to solution-path (default: src/api/dotnet)
  --api-dotnet-dockerfile <file> Backend (dotnet) Dockerfile, relative to api-dotnet-context (default: CsApi.Dockerfile)
  --api-app-prefix <prefix>      App Service name prefix for backend discovery (default: api-)
  --web-app-prefix <prefix>      App Service name prefix for frontend discovery (default: app-)
  --api-dotnet-app-prefix <pfx>  App Service name prefix for dotnet backend discovery (default: api-cs-)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --web-image-name) WEB_IMAGE_NAME="$2"; shift 2 ;;
        --web-context) WEB_CONTEXT="$2"; shift 2 ;;
        --web-dockerfile) WEB_DOCKERFILE="$2"; shift 2 ;;
        --api-image-name) API_IMAGE_NAME="$2"; shift 2 ;;
        --api-context) API_CONTEXT="$2"; shift 2 ;;
        --api-dockerfile) API_DOCKERFILE="$2"; shift 2 ;;
        --api-dotnet-image-name) API_DOTNET_IMAGE_NAME="$2"; shift 2 ;;
        --api-dotnet-context) API_DOTNET_CONTEXT="$2"; shift 2 ;;
        --api-dotnet-dockerfile) API_DOTNET_DOCKERFILE="$2"; shift 2 ;;
        --api-app-prefix) API_APP_PREFIX="$2"; shift 2 ;;
        --web-app-prefix) WEB_APP_PREFIX="$2"; shift 2 ;;
        --api-dotnet-app-prefix) API_DOTNET_APP_PREFIX="$2"; shift 2 ;;
...
# ----------------------------------------------------------------------------
# Resolve and validate SOLUTION_PATH (must contain src/)
# ----------------------------------------------------------------------------
SOLUTION_PATH="$(cd "${SOLUTION_PATH:-$PWD}" && pwd)"
if [[ ! -d "$SOLUTION_PATH/src" ]]; then
    echo "ERROR: SOLUTION_PATH '$SOLUTION_PATH' does not contain a src/ folder. Pass --solution-path pointing to the solution root." >&2
    exit 1
fi
...
if [[ -z "$API_APP_NAME" ]]; then
    dotnet_app="$(az webapp list --resource-group "$RESOURCE_GROUP" --query "[?starts_with(name, '${API_DOTNET_APP_PREFIX}')].name | [0]" -o tsv 2>/dev/null || true)"
    if [[ -n "$dotnet_app" ]]; then
        API_APP_NAME="$dotnet_app"
        BACKEND_RUNTIME_STACK="dotnet"
    else
        API_APP_NAME="$(az webapp list --resource-group "$RESOURCE_GROUP" --query "[?starts_with(name, '${API_APP_PREFIX}') && !starts_with(name, '${API_DOTNET_APP_PREFIX}')].name | [0]" -o tsv 2>/dev/null || true)"
        BACKEND_RUNTIME_STACK="python"
    fi
    if [[ -n "$API_APP_NAME" ]]; then
        write_success "Discovered Backend App : $API_APP_NAME (runtime: $BACKEND_RUNTIME_STACK)"
    else
        write_warn "No Backend App Service (${API_APP_PREFIX}* / ${API_DOTNET_APP_PREFIX}*) found in '$RESOURCE_GROUP'."
    fi
fi

if [[ -z "$WEB_APP_NAME" ]]; then
    WEB_APP_NAME="$(az webapp list --resource-group "$RESOURCE_GROUP" --query "[?starts_with(name, '${WEB_APP_PREFIX}')].name | [0]" -o tsv 2>/dev/null || true)"
    if [[ -n "$WEB_APP_NAME" ]]; then
        write_success "Discovered Frontend App : $WEB_APP_NAME"
    else
        write_warn "No Frontend App Service (${WEB_APP_PREFIX}*) found in '$RESOURCE_GROUP'."
    fi
fi
...
IMAGES=("${WEB_IMAGE_NAME}|${SOLUTION_PATH}/${WEB_CONTEXT}|${WEB_DOCKERFILE}")
if [[ "$BACKEND_RUNTIME_STACK" == "dotnet" ]]; then
    IMAGES+=("${API_DOTNET_IMAGE_NAME}|${SOLUTION_PATH}/${API_DOTNET_CONTEXT}|${API_DOTNET_DOCKERFILE}")
    BACKEND_IMAGE="${API_DOTNET_IMAGE_NAME}"
else
    IMAGES+=("${API_IMAGE_NAME}|${SOLUTION_PATH}/${API_CONTEXT}|${API_DOCKERFILE}")
    BACKEND_IMAGE="${API_IMAGE_NAME}"
fi
...
update_app_service_image "$WEB_APP_NAME" "$WEB_IMAGE_NAME"
